QUERY_ALL_PERSONS = """
select VTMA_DT_NASCIMENTO, floor(MONTHS_BETWEEN(CURRENT_DATE, VTMA_DT_NASCIMENTO) / 12) IDADE,
BDMT_BASE64, SNCA_DT_FATO, BAIR_LATITUDE, BAIR_LONGITUDE, BAIR_NM_BAIRRO,
CASE
    WHEN floor(MONTHS_BETWEEN(CURRENT_DATE, VTMA_DT_NASCIMENTO) / 12) IS NULL THEN
        (SELECT SIDP_NM_IDADE_APARENTE FROM SILD.SILD_IDADE_APARENTE WHERE SIDP_DK = VTMA_SIDP_DK)
        ELSE NULL END IDADE_APARENTE,
CASE
	WHEN floor(MONTHS_BETWEEN(CURRENT_DATE, VTMA_DT_NASCIMENTO) / 12) IS NULL THEN
	(SELECT SIDP_DK FROM SILD.SILD_IDADE_APARENTE WHERE SIDP_DK = VTMA_SIDP_DK)
ELSE NULL END INDICE_IDADE_APARENTE,
...
LEFT JOIN SILD_BLOB_DOCUMENTO ON SILD_SINDICANCIA.SNCA_DK = SILD_BLOB_DOCUMENTO.BDMT_DK
LEFT JOIN SILD_VITIMA ON SILD_VITIMA.VTMA_DK = SILD_SINDICANCIA.SNCA_DK
